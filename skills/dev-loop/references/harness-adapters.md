...
| `codex` | `codex exec --json --cd "{wt}" --sandbox workspace-write -c approval_policy=never --ephemeral --output-schema <file> -o <last.txt> "{obj}"` | positional | JSONL events; `-o` file holds the schema-validated final message | no max-turns → outer timeout | `--sandbox` / `-c approval_policy=` (`--ask-for-approval` is top-level only: `codex exec` rejects it); **`--cd` is the write fence, `--add-dir` is not (#24214)**; never `--full-auto` (overrides `--sandbox`); `.git/` read-only ⇒ host commits | exit 0 ∧ report |
