...
#
# Source it (safe to source repeatedly):   . environment/agy-keyring.sh
...
if ! agy_keyring_alive; then
    if ! command -v dbus-daemon >/dev/null 2>&1 || ! command -v gnome-keyring-daemon >/dev/null 2>&1; then
        echo "agy-keyring: dbus / gnome-keyring not installed — run environment/setup-antigravity.sh first" >&2
...
#   AGY_KEYRING_VERIFY=1 . environment/agy-keyring.sh
